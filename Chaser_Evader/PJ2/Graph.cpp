...
namespace Graph_lib {
	...
	*/
	...
	//draw box :: cannot fill color . it isn't realize  
void Box::draw_lines() const
{
		...
		fl_line(point(0).x + w, point(0).y + h - r, point(0).x + w, point(0).y + r);

		fl_arc(point(0).x + w - r * 2, point(0).y, r * 2, r * 2, 0, 90);
		fl_arc(point(0).x, point(0).y, r * 2, r * 2, 90, 180);
		fl_arc(point(0).x, point(0).y + h - r * 2, r * 2, r * 2, 180, 270);
		fl_arc(point(0).x + w - r * 2, point(0).y + h - r * 2, r * 2, r * 2, 270, 360);
}

void Regular_ploygon::draw_lines() const
{
	double const PI = 3.141592;
	double t = 360/nu;
	double a = sin(t*(PI/180));
	double b = cos(t*(PI/180));
	a *= ra;
	b *= ra;
	int xx = point(0).x + b;
	int yy = point(0).y - a;
	...
	if (color().visibility()) {
		...
		{
			if (i == 0){
				fl_line(point(0).x + ra, point(0).y, xx, yy);
				continue;
...
void Star::draw_lines() const
{
	...
	double a = sin(t*(PI / 180));
	double b = cos(t*(PI / 180));
	int xx = point(0).x + b * ra2;
	int yy = point(0).y - a * ra2;
	int xxx, yyy;
	// pointing a point to fill color 
	if (fill_color().visibility()) {
		t = 360 / (nu * 2);
		fl_color(fill_color().as_int());
		fl_begin_complex_polygon();
		for (int i = 0; i < (nu * 2); i++)
		{
			...
			{
				a = sin(t*(PI / 180));  	b = cos(t*(PI / 180));  // increas degree
				xx = point(0).x + (b * ra1);
				yy = point(0).y - (a * ra1);
				fl_vertex(xx, yy);
			}
			...
			{
				a = sin(t*(PI / 180)); b = cos(t*(PI / 180)); // increse degree
				xx = point(0).x + (b * ra2);
				yy = point(0).y - (a * ra2);
				fl_vertex(xx, yy);
			}
		...
		fl_vertex(xx, yy);
		fl_end_complex_polygon(); // end fill color
	...
	if (color().visibility()) {
		...
		{
			if (i == 0)
				fl_line(point(0.).x+ra1, point(0).y, xx, yy);
			else if (i % 2 == 1)    // if this now point is odd number
			{
				a = sin(t*(PI / 180));  	b = cos(t*(PI / 180));  // increas degree
				xxx = point(0).x + (b * ra1);   // calculate point.x
				yyy = point(0).y - (a * ra1);    // calculate point.y
				fl_line(xx, yy, xxx, yyy);
				xx = xxx; 		yy = yyy;   // save latest point
			}
			else if (i % 2 == 0)   // if this now point is even number
			{
				a = sin(t*(PI / 180)); b = cos(t*(PI / 180)); // increse degree
				xxx = point(0).x + (b * ra2);   //calculate point.x
				yyy = point(0).y - (a * ra2);    // calculate point.y
				fl_line(xx, yy, xxx, yyy );
				xx = xxx;	yy = yyy; // save latest point
			}
			t += (360 / (nu * 2));
