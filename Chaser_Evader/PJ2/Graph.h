...
struct Box : Shape {
	...
	{
		if (h <= 0 || w <= 0 || r <= 0)
			error("not positive values : we just need POSITIVE NUMBER");
		add(xy);
	}
...
private:
	int w, h, r; // w=width, h=height,  r=radius 
};


struct Regular_ploygon : Shape
{
	Regular_ploygon(Point xy, int r, int n) : ra{ r }, nu{ n }
	{
		if (n < 2 || r <= 0)
			error("NOT REGULAR VALUE");
		add(xy);

	}
	void draw_lines() const;

	int radius() { return ra; }
	int number_of_vertices() { return nu; }
	double degree() { return 360 / nu; }

private:
	int ra;   // radius
	int nu; // number of vertices
};

struct Star : Shape
{
	Star(Point xy, int r1, int r2, int n) : ra1{ r1 }, ra2{ r2 }, nu{ n }
	{
		if (r1 < 0 || r2 < r1 || n < 2)
			error("NONONONONONONONONONONONONO ");
		add(xy);
	}
	void draw_lines() const;

	int radius1() { return ra1; }
	int radius2() { return ra2; }
	int degree() { return 360 / nu; };

private:
	int ra1, ra2;   // radius 1, 2
	int nu;			// number of vertices
